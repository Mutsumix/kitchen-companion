...
## 10. 音声往復(縦一本): プッシュ・トゥ・トーク → Worker → スピーカー再生

プロジェクトの最優先マイルストーン。デバイス側は [src/main.rs](../src/main.rs)、クラウド側は9章。

1. `talk()`: 録音PCMをWAVとしてHTTPS POST(`crt_bundle_attach` でTLS証明書バンドル、タイムアウト30秒)、応答の16kHz PCMを読み切る
2. 操作: **押している間録音(上限15秒)→ 離すと送信 → 応答をスピーカー再生**。0.5秒未満はキャンセル
3. 応答再生はモノラル→ステレオ複製して既存の非ブロッキング再生キューへ(末尾無音はWorker側で付加済み)

- **つまずき(最重要)**: `talk()` の4KBバッファでメインタスクのスタック(8192)があふれ、起動時クラッシュ。`CONFIG_ESP_MAIN_TASK_STACK_SIZE=20000` が必須。コンパイラバグと誤診した顛末は[troubleshooting](troubleshooting.md)
- **つまずき**: I2S非ブロッキング書き込みの `ESP_ERR_TIMEOUT` は「DMA満杯」の意味でエラーではない。expectで落とすと不安定な再起動になる
- **つまずき**: Xtensaツールチェーンは `espup update` ではstableまでしか上がらない。`espup install --toolchain-version 1.97.0.0` で明示指定(1.95.0.0にはLLVM ICEバグあり)
- **一次情報**: ESP-IDF「Performance/ Stack sizes」+ esp-idf-templateのsdkconfigコメント / esp-idf-hal `src/i2s.rs` の `remap_result`(部分書き込みの扱い)
- **結果**: 2026-07-20実機貫通。発話13.8秒→往復8386ms(Worker内 stt=1941/llm=727/tts=2115ms、残りはTLS+転送)

## 11. 緊急復旧手順(画面真っ黒・反応なしのとき)
